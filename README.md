# Surge iOS Privacy + Push R13.4 Strict DNS
...
3. Apple 公共 Wi-Fi 门户探测先直连，STUN 随后进入隐藏的 `UDP` 组。
4. 16 个经审阅的大陆应用 DNS 主机先进入隐藏的 `Domestic`，默认直连；临时取消隐藏后仍可切到 `Proxy`。
...
13. 剩余域名与连接落入 `FINAL,Final,dns-failed`；`Final` 默认 `Proxy`，域名由代理侧解析。

Surge 采用首条命中。两个内容正确的规则文件只要位置颠倒，结果也会改变。R13.4 因此把大陆 DNS 例外、通用端口拒绝、境外 DNS、精确域名集、带 `no-resolve` 的 CN GeoIP 和最终代理兜底同时写进配置审计和故障注入测试。
...
4. 若从旧版覆盖升级，先在私人副本中临时取消四个辅助组的隐藏，确认 `AdBlock=REJECT`、`Security=REJECT`、`UDP=Proxy`、`Domestic=DIRECT`，再恢复 `hidden=1`。Surge 可能按组名保留旧选择，单纯隐藏不会重置历史状态。
...
`NodePool` 保持 `hidden=0`，继续承担订阅检查和手动节点选择。`ApplePush`、`AdBlock`、`Security`、`UDP` 和 `Domestic` 使用 `hidden=1`；后四组的定义、默认成员和规则引用完整保留，需要排错时只在私人副本中临时取消隐藏。
...
这不是全局匿名 DNS 设计。明确走 `Domestic` 或需要本地判定的连接仍可使用 AliDNS 与 DNSPod，且 `encrypted-dns-follow-outbound-mode=false` 使 Surge 自身的 DoH 直连。R13.4 的严格边界是不给未命中域名在 CN GeoIP 处强制做这次本地解析；它们默认以主机名进入 `Final/Proxy`，由代理侧解析。这样同时避开域名型代理节点的启动解析环。
...
大陆解析器包括 AliDNS、DNSPod、360 DNS、百度/字节相关解析域名及清单中的其他固定入口，它们进入隐藏的 `Domestic`，默认 `DIRECT`。这保留 R13.3 对国内软件卡顿的定向修正，避免应用自带的大陆 DoH/DoT 无条件绕海外代理。若所在网络限制这些入口，可在私人副本中暂时把 `Domestic` 改为 `hidden=0`，再整体切到 `Proxy`。
...
R13.4 不再借末端 CN GeoIP 为未命中域名触发本地 DoH。网页出口在境外而解析器仍显示中国移动、阿里或其他国内服务时，先确认已重新载入 R13.4、清除旧 DNS 与网页缓存并停用会插入规则的模块；随后切换另一个已知节点复测。若结果跟随节点变化，来源是节点服务端 DNS，客户端无法替远端节点重写解析器。

页面直接显示本机公网 IPv4 或 IPv6 时，应检查私有模块、`include-all-networks`、`ipv6-vif`、`UDP` 选择和最近请求路径。不要通过关闭 IPv6 掩盖尚未定位的路由问题。

### 模块优先级

Surge 模块可以覆盖 General 项，并把 Rule、Host、Script 和 Rewrite 内容插入主配置之前。一个缺少 `no-resolve` 的模块规则集、提前直连的检测域名或改写 DNS 的模块，都可能在 R13.4 规则生效前改变结果。
...
STUN 在公网 DNS 端口、公共域名和公网 IP 规则之前进入 `UDP`。该组默认选择 `Proxy`，让 STUN 跟随当前主代理。`UDP` 只在控制面板隐藏；需要排查时，在私人副本中临时改为 `hidden=0`，即可在 `Proxy`、`NodePool`、`REJECT` 和 `DIRECT` 之间选择。`DIRECT` 会暴露真实公网地址，只适合短时确认兼容性。
...
17. 不触发解析的 `GEOIP,CN,Domestic,no-resolve`、公网 IPv4/IPv6 字面量兜底和 `Final/Proxy` 域名兜底。
...
- Global 必须位于 `GEOIP,CN,Domestic,no-resolve` 之前；该 GeoIP 规则不得去掉 `no-resolve`，否则未命中域名会重新触发本地 DNS。
...
R13.2 起引用三份 SukkaW 维护资源，R13.4 原样保留 URL、类型、策略和更新间隔。它们固定到精确域名与路径，不使用仓库 `main` 拼接地址，但内容会按上游发布变化。

| 运行 URL | 类型 | 策略 | 更新间隔 | 发布时活动条目 |
| --- | --- | --- | --- | ---: |
| `https://ruleset.skk.moe/List/domainset/reject_phishing.conf` | `DOMAIN-SET` | `Security` | 86,400 秒 | 147,474 |
| `https://ruleset.skk.moe/List/domainset/reject.conf` | `DOMAIN-SET` | `AdBlock` | 86,400 秒 | 135,224 |
...
动态源提高广告、钓鱼和国内规则的时效，也引入上游变化风险。`AdBlock`、`Security` 和 `Domestic` 在 R13.4 中隐藏但没有删除；发生误报时，可在私人副本中把对应组临时改为 `hidden=0`，切换策略确认后再查看最近请求和上游变化。定期检查任务只报告变化，不会自动改写配置或订阅。
...
关键词、通配符和大型动态域名集都可能产生误拦截。遇到网页或应用异常时，可先在私人副本中把 `AdBlock` 临时改为 `hidden=0`，再切到 `DIRECT` 复测。确认命中项以后再调整列表，不要长期关闭所有广告处置来掩盖单条误判。
...
这些平台可以托管不同租户的内容。R13.2 起不再把它们永久写死为 `Proxy` 或 `DIRECT`，而是统一交给 `Domestic`。R13.4 只隐藏该组，默认仍直连；在境外、校园网或受限网络中，可先临时改为 `hidden=0` 后整体切到 `Proxy`。专用服务规则仍在它们之前完成匹配。
...
| `MIGRATION.md` | R13.3 到 R13.4 的迁移说明 |
| `CHANGELOG.md` | 版本变化记录 |
...
可以把未解压的 `Surge-R13.4-Complete-No-Embedded-20260828.zip` 放在仓库根目录，并保留 `.github/workflows/install.yml`。随后在 Actions 中手动运行 `Install and audit Surge R13.4`，在必填的 `archive_sha256` 输入框填写包外公布的整包 SHA-256。
...
先确认配置标题为 R13.4，检测站点走 `Proxy`，并查看它最终选择的节点。重新载入配置后清理 Surge DNS 缓存和网页数据，在无额外模块的条件下复测。R13.4 的 `GEOIP,CN,Domestic,no-resolve` 不会为未命中检测域名触发本地 DoH。

若最近请求显示模块 `RULE-SET` 触发本地 DNS，关闭对应模块并重新载入。模块规则位于主配置之前，主配置无法在后面撤销已经发生的解析。若清缓存后解析器随代理节点变化，则是节点服务端 DNS；换节点或由节点提供方调整。
...
先确认当前 Smart 或手动节点支持 UDP，再查看 STUN 是否进入隐藏的 `UDP`。需要逐项复测时，在私人副本中暂时把该组改为 `hidden=0`，再在 `Proxy`、`NodePool`、`REJECT` 和 `DIRECT` 之间选择。
...
- 临时把 `Domestic` 改为 `hidden=0`，在境内和受限网络分别测试 `DIRECT` 与 `Proxy`，测试后恢复隐藏。
- 临时取消 `AdBlock` 和 `Security` 的隐藏并切换处置策略，确认紧急关闭路径有效，测试后恢复隐藏。
...
- 末端 `GEOIP,CN,Domestic,no-resolve` 不为未命中域名触发本地解析；严格 DNS 边界的代价是未被现有国内域名规则覆盖的中国服务可能落入 `Final/Proxy`，需要按最近请求补充精确规则。
...
- [ ] `Proxy` 与隐藏的 `UDP` 仍默认使用 `AllServer`/`Proxy` 主链路
- [ ] `AllServer` 与五个地区组仍为 `smart`，没有无效 `interval` 或 `tolerance`
- [ ] `ApplePush`、`AdBlock`、`Security`、`UDP` 和 `Domestic` 均为 `hidden=1`，但定义、成员和规则引用完整保留
