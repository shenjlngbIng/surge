...
| Fast | url-test，隐藏 | 在全订阅节点中选择最近一轮测试的最低有效延迟；仍可通过 Proxy 使用 |
...
Fast 的独立卡片已隐藏，自动测速、`Proxy → Fast` 引用和 Fail-Closed 保护继续有效。若需临时显示，删除 Fast 行的 `hidden=1` 或改为 `hidden=0`。[隐藏参数](https://manual.nssurge.com/policy-groups/parameters.html)
