...
## 2026-09-09 R13.25 国内哔哩哔哩分流修复

- 按要求将配置文件头替换为作者、仓库、更新日期三行。
- 国内 B 站规则由 16 条增至 27 条，补齐视频、图片及 PCDN 漏项；共享 CDN 仅放行指定主机。
- 国内 B 站规则前移至广告规则之前；国际站 Proxy 例外保留，并补充 SNI/Host 匹配。
- 发布规则快照 `4057ea435ece7617cabec5b7ac5a49c1d93937de`，同步来源记录、README、迁移说明、审计与校验和。
- 增加 202 项 B 站分流及 3 项退化检查；常见评论 API 原本直连，未将离线通过表述为真机卡顿已全部解决。

## 2026-09-09 R13.25 Fast 卡片隐藏

- 为 Fast 增加 `hidden=1`，隐藏独立策略卡片；保留 Proxy 中的 Fast 选项、自动测速、0 容差和 Fail-Closed。
- 策略组仍为 41 个，可见 29 个、隐藏 12 个；服务地区、分流规则、推送、DNS 和 UDP 不变。
- 同步说明、可见性审计、运行锁和发布校验和。

## 2026-09-09 R13.25 服务地区自动选点与文档发布

- 将 19 个服务组改为 url-test，其中 10 个限制地区；ChatGPT/Claude/Gemini 使用现有美、日、新、台节点，HBO 按 HBO Max 范围处理，巴哈延续台湾片库取向。Apple 默认直连保留。
- 受限服务直接复制严格地区源成员，移除经 Proxy/Auto 或外层地区入口跨区的路径；通用地区 fallback 仍保留。地区正则排除现有地区标签冲突及中转/箭头名称，识别结果不作为真实出口证明。
- 纳入此前 R13.24 的 Fast 入口、0 容差与 300 秒结果有效期，并在订阅入口统一测试地址及 5 秒超时，保留 UDP 中继。
- 新增 15 条流媒体地区规则，调整 viu.now.com 出口，分开 Hulu 美国站、TVer、香港 Now 等平台；主规则增至 157 条，策略组为 41 个。
- 配置注释精简，README 按实际配置全面重写，升级说明、审计记录和维护工具同步。相对已交付的 R13.25 文件，注释修订不改变有效指令。
- 审计补充服务地区边界、错误地区回退和媒体改向退化测试；运行锁升级 schema 36，发布清单及校验和重新生成。Fail-Closed、APNs、DNS、UDP 和 29 份固定规则快照保留。

## 2026-09-08 R13.24 最低延迟入口（随 R13.25 发布）

- 新增 Fast，作为 Proxy 的首选；原 Auto Smart、NodePool 与五个通用地区入口保留。
- 五个地区测速源改为 300 秒、0 容差、首次等待评估并抑制切换通知；订阅节点采用统一 Cloudflare HTTP 204 与 5 秒超时。
- 保留 Fail-Closed，明确其失败显示为预期。此版本此前作为配置文件交付，未单独写入仓库。

## 2026-09-08 R13.23 推送与规则下载恢复

- ApplePush 改为 `Proxy → Auto → DIRECT`，手选节点不可用时先尝试健康备用代理，保留全部代理不可用时的直连回退。
- GitHub raw 的前置分流改用 Auto，避免规则下载跟随 Proxy 的失效手动选择；不增加直连或第三方镜像。
- 保留 APNs 接管所需的 include-all-networks 与 include-apns，说明兼容性警告及系统推送长连接的验收方法。
- 补充 APNs/Telegram 域名、IP 边界、SNI、资源下载与故障恢复测试，配置故障注入增至 107 项。运行锁升级为 schema 35。
- 29 份外置列表及固定地址、40 个组、142 条主规则、唯一“桔子”订阅、隐藏辅助组、哨兵、DNS 和 UDP 参数保持。

## 2026-09-08 R13.22 缩短订阅源名称

- 将隐藏订阅源 `Subscription` 改为 `桔子`，同步节点池、Auto 和五个地区源的 7 处引用。配置中搜索 `桔子 =` 即可定位唯一订阅入口。
- 同步使用说明、检查工具和运行锁，仅改变组名，保留全部参数、节点筛选、保护成员和分流规则。
- 29 份外部规则及其固定提交不变，沿用 R13.21 的广告误拦和加密 DNS 顺序修正。

## 2026-09-08 R13.21 广告误拦与加密 DNS 顺序修正

- 移除 Ads 中 8 条正常业务网站的宽泛关键词，将历史来源标记改为注释，活动条目由 152 降至 143。保留已有细分广告规则及来源许可。
- 境外 DNS 域名规则前移至 853/8853 端口拒绝之前，保留原有 53 端口拒绝及前置国内、本地例外，不新增解析端点或公共 IP 放行。
- 先提交修订规则，再把 29 份外置资源锁定到提交 `6e8e1bfbbdda66ee8ad0a5ad3979b6de8b5b7a51`。仅 Ads 内容变化，其余 28 份列表保持原样；运行锁升级到 schema 34。
- 保留 40 个策略组、单订阅入口、地区回退、共享登录依赖、UDP 参数及哨兵，主配置仍含 142 条分流指令。
- 增加 16 个正常网站、8 个广告、132 个 DNS 端口案例及 10 个行为退化案例；配置故障注入增加到 104 项，并先验证正常基线，防止审计器全部拒绝却被误判通过。
- README、升级说明和审查记录同步更新，测试结果不作为真实 UDP 或服务解锁承诺。

## 2026-09-08 R13.20 文档重写与配置精简

- 按当前配置重写 README，补齐安装、策略组和服务映射、分流顺序、DNS、UDP、哨兵边界、资源更新及故障排查说明。
- 将配置内注释改为简短中文，删除重复文件名说明、旧修复叙述和不准确的参数解释，保留作者与来源信息。
- 删除 40 处 no-alert=0、29 处 hidden=0、34 处 include-all-proxies=0 和 8 项与官方默认值相同的全局设置。
- 删除 dns.alidns.com 与 dns.nextdns.io 的重复精确规则，对应同出口后缀规则保留。
- 主配置从 387 行、22,502 字节降为 311 行、17,316 字节；分流指令 144 条降为 142 条，40 个策略组和 29 份外部规则不变。
- 保留单订阅入口、隐藏辅助组、Smart、节点池、地区回退、服务选择、UDP 参数与哨兵，不内嵌规则，不修改私人订阅或 Sub-Store。
- 同步运行锁、审计及发布工具，102 项故障注入与 23 个域名/SNI 案例通过；离线结果不作为真机 UDP 成功承诺。

## 2026-09-07 R13.19 辅助组界面修订

- 隐藏 AdBlock、Security、UDP、Domestic 四个辅助组；保留其默认选项、功能和全部分流。节点池、Auto、地区及服务组不变。
- 订阅入口、UDP 参数、DNS 和 29 个外部规则引用均未改变；不内嵌规则、不修改 Sub-Store。
- 补充 Fail-Closed 的预期失败与真实节点 UDP 诊断的区别：空白 UDP 区域没有提供转发成功或失败的证据。
- 增加四个辅助组可见性回归用例，避免后续生成时重新显示；隐藏界面不等于修复或验证真实 UDP 连接。

## 2026-09-07 R13.19 外部规则与完整保护链

- 按用户要求撤销 R13.18 的内嵌方式；29 份规则恢复外部引用，改用固定提交的 GitHub raw URL，移除内嵌生成命令。
- 新增隐藏 Subscription 源并保护空源；NodePool 默认 Auto 且保留手动节点，Auto 恢复代理类型 Fail-Closed 哨兵，地区空源保留 REJECT。没有循环引用或新的用户设置步骤。
- AdBlock、Security、Domestic 接入对应规则；UDP 控制 STUN 与未命中前置规则的 UDP。订阅导入启用 UDP 转发开关，服务端能力仍需实际验证。
- 删除独立直连 DoH 模式下不工作的五条 PROTOCOL DNS 指令，保留证书校验、引导和 DNS 端口边界。
- 40 个策略组、144 条主配置分流指令、29 个外部资源、零内嵌列表；29 个资源 GET / SHA-256、19 个分流案例、地区/空池模型和 88 项故障注入通过。
- Sub-Store 由用户处理，本版不改其模块或后端；不生成可能覆盖已修好地址的旧订阅副本。离线测试不作真机连通性承诺。


## 2026-09-07 R13.18 单文件规则

- 将 29 个原始规则快照的 5,546 条规则内置到 Surge.conf。主配置总计 5,664 条规则，无外部 RULE-SET/DOMAIN-SET 下载。
- 保留 39 个策略组、唯一订阅入口、原有分流顺序和 DNS 设置。五个隐藏地区源增加原生 REJECT，避免地区空源被替换为 DIRECT。
- 新增可重复生成的内置规则工具，逐条验证策略/匹配/解析语义，覆盖 16 个离线分流案例与 6 个损坏案例。
- 更正前版“Smart 空组天然失败关闭”与“PROTOCOL DNS 覆盖任意应用 DoH”的错误结论；当前配置仍不保证完整自动断网哨兵。
- Sub-Store HTTP 500、节点连接错误及当前订阅有效性仍需手机日志；旧到期时间不能作为未续费的证明。无真机验证时不再宣称全部恢复。
- 下方历史条目保留原发布记录，其中与本节及当前审计报告冲突的结论已撤回。
