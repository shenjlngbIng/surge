...
本仓库发布 Surge iOS Privacy + Push R12.16 配置及维护工具。`Surge.conf` 中的第三方规则版权归各自作者或项目所有，相关许可证副本位于 `THIRD_PARTY_LICENSES/`。
...
R12.16 对固定上游快照执行本地、可复现的语义筛选。Bilibili 国内版与国际版使用两个规则文件和现有策略，国内版直连，国际版进入 `Streaming`。共享云、遥测和国内服务误代理项已经删除，Netflix 的宽泛云网段改为官方 Open Connect ASN。所有排除项和本地补充均记录在 `Rules/upstreams.lock.json`。
